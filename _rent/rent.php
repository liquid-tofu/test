...
class RentHome extends HometownBoard {
  public function soul($id = null, $name = null) {
    $search = isset($_GET['search']) ? trim($_GET['search']) : '';

    $result = $this->conn->prepare("CALL selectAllFromRent(:search)");
    $result->bindValue(':search', $search);
    $result->execute();
    return $result->fetchAll(PDO::FETCH_ASSOC);
  }
}
$immortal_gu = new RentHome((new WeavingThreat())->connect(), $tbl_name);
...
<body class="mt-5 mb-5">
  ...
  <div id="tbl-con" class="container-md d-flex flex-column align-items-center justify-content-center text-center py-3 gap-3 rounded-3">
    ...
    <!-- configure -->
    <?php $immortal_gu->board('id', 'name', $columns, "editrent.php"); ?>
