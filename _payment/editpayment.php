...
try {
  ...
  $stmt->bindValue(':table', 'tblpayment');
...
if (isset($_POST['submit'])) {
  ...
  try {
    $stmt = $conn->prepare("CALL updateForPay(:code, :price, :month, :year, :id)");
...
<div class="container border border-primary rounded p-5 w-100">
  ...
  <form method="post">
    ...
    <div class="mb-3">
      ...
      <input type="number" step="0.01" min="0" max="1000000" name="price" class="none-btn form-control" placeholder="Enter price" value="<?= htmlspecialchars($row['price'] ?? '') ?>">
    </div>
    ...
    <div class="mb-3">
      ...
      <input type="number" step="1" min="2000" max="2030" name="year" class="none-btn form-control" placeholder="Enter year" value="<?= htmlspecialchars($row['year'] ?? '') ?>">
    </div>
